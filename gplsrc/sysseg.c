...
bool start_qm() {
  ...
  const char* pid_path = (const char*) (sysseg->pid_file_path [0] ?
    sysseg->pid_file_path : default_pid_path);
  ...
  if (fork() == 0) { /* Child process */
    ...
    snprintf(errmsg_qmlnxd, sizeof (errmsg_qmlnxd), "Error %d starting %s!",
      errno, path);
    log_message (errmsg_qmlnxd);
  } else { /* Parent process */
    /* Waiting qmlnxd is started, time-out in 1s */
    int sleep_count;
    for (sleep_count = 0; sysseg->qmlnxd_pid == -1 && sleep_count < 100;
      ++sleep_count) {
      usleep (10000); /* 10 ms */
    }
    ...
    if (shm.shm_nattch) {
      if ((sysseg = (SYSSEG*)shmat(shmid, NULL, 0)) != (void*)(-1)) {
        ...
        if (sysseg->qmlnxd_pid) {
          ...
          unlink((const char*) (sysseg->pid_file_path [0] ?
            sysseg->pid_file_path : default_pid_path));
        }
